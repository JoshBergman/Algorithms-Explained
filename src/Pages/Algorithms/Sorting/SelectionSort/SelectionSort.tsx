...
export default function SelectionSort() {
  ...
  return (
    <AlgoPageTemplate algo={algo} title={pageTitle} buttonContainer={buttons}>
      ...
      <P>
        Selection sort is a simple and intuitive algorithm used to sort a list
        of elements in ascending or descending order. It works by repeatedly
        finding the smallest element in the unsorted part of the list and moving
        it to the beginning of the sorted part of the list.
      </P>
      ...
      <P>
        To begin, the algorithm starts with the entire list as the unsorted part
        and an empty list as the sorted part. Then, it finds the smallest
        element in the unsorted part of the list and moves it to the beginning
        of the sorted part of the list. This process is repeated for each
        element in the unsorted part of the list, until the entire list is
        sorted.
      </P>

      <P>
        To find the smallest element in the unsorted part of the list, the
        algorithm compares each element in the unsorted part of the list with
        the current smallest element, which is initially set to the first
        element of the unsorted part of the list. If a smaller element is found,
        the current smallest element is updated to be the new smallest element.
      </P>

      <H>Time Complexity</H>
      <TimeTable best={"O(n²)"} average={"O(n²)"} worst={"O(n²)"} />
      <P>
        The time complexity of selection sort is O(n<sup>2</sup>), where n is
        the number of elements in the input list. This means that the time it
        takes to sort the list increases quadratically with the number of
        elements in the list. In the worst-case scenario, where the input list
        is in reverse order, selection sort will make n<sup>2/2</sup>{" "}
        comparisons and n swaps.
      </P>

      <P>
        However, unlike some other sorting algorithms, selection sort always
        makes the same number of comparisons and swaps, regardless of the input.
        This predictability makes it useful for teaching beginners about sorting
        algorithms, but it also makes it less efficient than some other
        algorithms. In general, selection sort is best suited for small lists or
        for lists that are already partially sorted. For larger or more complex
        lists, more efficient sorting algorithms, such as merge sort or
        quicksort, are usually preferred.
      </P>
    </AlgoPageTemplate>
  );
}

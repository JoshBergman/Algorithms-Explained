...
export default function Array() {
  ...
  return (
    <AlgoPageTemplate algo={algo} title={pageTitle} buttonContainer={buttons}>
      ...
      <P>
        Insertion sort is a simple and intuitive algorithm used to sort a list
        of elements in ascending or descending order. It works by dividing the
        list into two parts: a sorted part and an unsorted part. At the
        beginning, the sorted part is empty, and the unsorted part is the entire
        list.
      </P>

      <P>
        The algorithm iterates through the unsorted part of the list, one
        element at a time, and inserts each element into its correct position in
        the sorted part of the list. To do this, it compares the current element
        with the elements in the sorted part of the list, starting from the
        right end, until it finds the correct position to insert the current
        element.
      </P>

      <P>
        The algorithm repeats this process for each element in the unsorted part
        of the list until the entire list is sorted. At the end of each
        iteration, the last element in the sorted part of the list is the
      ...
      <P>
        The time complexity of insertion sort is O(n<sup>2</sup>), where n is
        ...
        is in reverse order, the algorithm will make n<sup>2/2</sup> comparisons
        and n<sup>2/2</sup> swaps. However, if the input list is already
        partially sorted, insertion sort can have a best-case time complexity of
        O(n), where only n-1 comparisons and 0 swaps are needed to sort the
        list. In general, insertion sort performs well on small lists or nearly
        sorted lists, but becomes less efficient on larger, unsorted lists. As a
        result, it is typically not used for large-scale sorting applications,
        where more efficient algorithms such as quicksort or mergesort are
        preferred.
      </P>
    </AlgoPageTemplate>
  );

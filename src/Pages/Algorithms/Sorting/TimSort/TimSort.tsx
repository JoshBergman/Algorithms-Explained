...
^
  ...
  return (
    <AlgoPageTemplate algo={algo} title={pageTitle} buttonContainer={buttons}>
        ...
        <P>TimSort is a sorting algorithm that was introduced in 2002 by Tim Peters, a software developer at Python Software Foundation. 
            TimSort is a hybrid sorting algorithm that combines elements of <RefLink to="/insertionsort" >insertion sort</RefLink> and <RefLink to="/mergesort" >merge sort</RefLink> to achieve a fast, stable, and efficient sorting algorithm. 
            The algorithm works by first dividing the input list into small sub-lists, sorting them using insertion sort, and then merging the sorted sub-lists using merge sort. 
            One of the key advantages of TimSort is that it is able to take advantage of any preexisting order in the input list, making it an ideal choice for sorting partially ordered lists.
        </P>

        <P>
        To understand TimSort, imagine that you have a deck of cards that you want to sort. 
        You could start by dividing the deck into small piles, each containing a few cards. 
        Next, you could sort each pile individually using insertion sort, which is a simple sorting algorithm that works well for small lists. 
        Finally, you could merge the sorted piles back together using merge sort, which is a more sophisticated algorithm that can efficiently merge two sorted lists into one sorted list. 
        By combining these two algorithms, TimSort is able to achieve a faster and more efficient sorting algorithm than using either algorithm alone. 
        Additionally, TimSort is stable, meaning that it preserves the relative order of equal elements in the input list, making it ideal for applications where maintaining the original order of equal elements is important.
        </P>
